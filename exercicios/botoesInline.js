...
bot.start( async ctx => {
    ...
    await ctx.reply(`A contagem atual está em: ${contatgem} \n`, botoes)
} )

bot.action(/add (\d+)/, ctx => {
    contatgem += parseInt(ctx.match[1]);
    ctx.reply( `A contagem atual está em ${contatgem} \n`, botoes )
})
...
bot.action('reset', ctx => {
    contatgem = parseInt(0);
    ctx.reply( `A contagem atual está em ${contatgem} \n`, botoes )
})

bot.action('result', ctx => {
    ctx.answerCbQuery( `Acontagem atual estám em ${contatgem} \n`, botoes )
})
